...
type HealthProfile = {
  ...
  heartRate: number; steps: number; sleep: number; calories: number;
};
...
function Stat({ icon, label, value }: { icon: string; label: string; value: string }) {
  return (
    <View style={s.stat}>
      <Text style={s.statLabel}>{icon} {label}</Text>
    ...
    <View style={s.wrap}>
      <View style={s.profile}>
        ...
        <View>
          ...
          <Text style={s.age}>{data.age} years old</Text>
          <Text style={s.location}>{data.location}</Text>
        </View>
      </View>
...
export function HealthCard({ delay, reloadKey }: { delay: number; reloadKey: number }) {
  ...
  return (
    <View>
      <SkeletonTheme animation="beat" revealOnExit color="#3f3f46" highlightColor="#71717a" borderRadius={8} minDuration={3000}>
...
const s = StyleSheet.create({
  ...
  name:      { fontSize: 16, fontWeight: '700', color: '#f4f4f5', marginBottom: 3 },
  age:       { fontSize: 12, color: '#71717a', marginBottom: 3 },
  location:  { fontSize: 12, color: '#52525b' },
  statsRow:  { flexDirection: 'row', gap: 8 },
  stat:      { flex: 1, backgroundColor: '#18181b', borderRadius: 10, padding: 12 },
